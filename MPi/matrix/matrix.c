...
To compile: mpicc -o matrix_mul matrix_mul.c 
 compile command : mpicc matrix.c -o matrix   and run command : mpirun -np 4 ./matrix
...
mpi install step : open folder -> new terminal
-> sudo apt update
-> sudo apt install openmpi-bin openmpi-common libopenmpi-dev
-> mpicc --version
   mpirun --version
-> matrix_mul.c  //file create
-> mpicc matrix_mul.c -o matrix_mul
-> mpirun -np ./matrix_mul
